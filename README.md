...
곡이 바뀌면 웨이브 바·진행 바·볼륨 바·재생 버튼·셔플/반복/좋아요 버튼·연결 표시등의
강조색이 **앨범 표지에서 뽑은 색**으로 함께 바뀐다. 덱이 "지금 이 곡"을 입는다.
...
| 문제 | 대응 |
|---|---|
| 면적으로만 고르면 거의 모든 표지에서 배경(검정/흰색)이 뽑힌다 | 채도를 제곱해 가중한 점수로 선택 |
| 어두운 표지의 색은 `#0A0A0A` 배경에서 안 보인다 | 명도 하한 165 적용 (색상은 유지) |
| 형광 표지를 그대로 쓰면 눈이 아프다 | 채도 상한 225 적용 |
| 흑백 표지에서 회색을 뽑으면 강조색 역할을 못 한다 | 유채색이 없으면 기본 그린으로 복귀 |

색상환을 24구간으로 나눠 비슷한 색을 묶고, 각 구간의 `채도² × 명도` 합계가
가장 큰 구간의 평균색을 쓴다. 48×48로 축소해 계산하므로 640×640 이미지가 **약 5ms**다.
곡이 바뀔 때만 실행되고, 같은 색이면 UI를 다시 칠하지 않는다.

### 하드웨어 이식 메모

포인트 색이 앨범마다 바뀌므로 저가 TN LCD에서는 색에 따라 대비가 달라질 수 있다.
`app/ui/palette.py`의 `TARGET_MIN_VALUE`(명도 하한)를 올리면 패널에 맞춰 조정된다.
...
```
┌──────────────────────────────────────────────┐
│ ◉                             기기명 · 컴퓨터 │  Spotify 마크 = 연결 상태
│ ┌────────┐  Sunset Lover                     │
│ │  ART   │  Petit Biscuit                    │
│ │160×160 │  SUNSET LOVER                     │
│ └────────┘  ▁▃▅▇▅▃▁▂▄▆                      │
│ ━━━━━━━━━━━●━━━━━━━━━━━━━━━━━━━━━━━━━━━━   │  진행 바 (전체 폭)
│ 1:42                                    3:58 │
│  ♡   ⤨   ◀   ( ▶ )   ▶▶   ⟲   ⋯           │  컨트롤 한 줄
...
**세로 스택 구조.** 2열로 나누면 앨범 아트가 화면 절반을 먹어 나머지가
눌린다. 아트를 작게 두고 위에서 아래로 쌓으면 진행 바와 컨트롤이
전체 폭을 쓸 수 있다. PC 버전도 같은 구조에 치수만 다르다.

`⋯` 메뉴에 음소거·볼륨·새로고침·종료가 들어 있다. 기기에는 로터리
엔코더가 볼륨을 담당하므로 화면에서 슬라이더를 뺐고, 그만큼 아트를 키웠다.

Spotify 마크가 연결 상태를 겸한다 — 정상이면 앨범 강조색, 끊기면 빨강.

### 실제 기기용 실행 옵션

```powershell
# 실물 4인치 크기로 보기 - 모니터 DPI를 읽어 축소한다
python main.py --compact --true-size
...
python main.py --compact --true-size      # 실물 4인치 크기로 표시
